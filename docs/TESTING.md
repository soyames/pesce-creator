...
5c. Paramètres → **« Rediriger les articles déjà publiés »** : le compte rendu indique les textes rapatriés, les pages Telegraph mises à jour et les messages du canal redirigés. Vérifier ensuite qu'un **ancien** article s'ouvre et se lit **intégralement** dans le Mini App (avant le rattrapage, seul son titre s'affichait). Relancer n'ajoute jamais de doublon et n'écrase jamais un texte déjà enregistré ; les publications postées à la main depuis Telegram ne sont pas modifiables par le bot et sont comptées comme telles.
5d. Dans **« Lecture Article »**, appuyer sur un onglet de la barre du bas (Écrits, Directs, Photos, Soutenir) : le lecteur se referme et la section demandée s'ouvre réellement.
