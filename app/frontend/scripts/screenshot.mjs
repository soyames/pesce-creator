...
const SHOTS = [
  ...
  ['ecrits-citations-390', 390, 844, `http://127.0.0.1:${PREVIEW_PORT}/?preview=1#ecrits-citations`, `document.getElementById('publicationFeed').children.length > 0 && document.getElementById('publicationFeed').dataset.loading !== 'true'`],
  ['reader-citation-390', 390, 844, `http://127.0.0.1:${PREVIEW_PORT}/?preview=1#post-13`, `!document.getElementById('reader').hidden && document.getElementById('readerContent').children.length > 0`],
  ...
  // Le pupitre en mode CITATIONS : ni titre ni média, le champ auteur à la place. La condition de
  // prêt clique la pastille (idempotent) et n'est vraie qu'une fois la bascule réellement appliquée.
  ['studio-web-citations-1280', 1280, 900, `http://127.0.0.1:${PREVIEW_PORT}/studio?preview=webstudio`, `(function () { var pill = [].slice.call(document.querySelectorAll('.format-pill')).filter(function (b) { return b.dataset.format === 'Citations'; })[0]; if (pill) pill.click(); var tab = document.querySelector('[data-web-tab="rediger"]'); var title = document.getElementById('titleField'); var attr = document.getElementById('attributionField'); if (!pill || !title || !attr) return false; var ready = getComputedStyle(title).display === 'none' && getComputedStyle(attr).display !== 'none'; if (ready && tab) tab.click(); return ready; })()`],
