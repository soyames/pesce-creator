...
test('studio : les actions d\'image/d\'audio/de vidéo restent fail-closed sans session', async () => {
  ...
  await withEnv(baseEnv, async () => {
    for (const action of ['article_image_upload', 'article_image_from_channel', 'article_update', 'telegraph_footer_backfill', 'video_publish', 'video_update', 'audio_publish', 'resync_message', 'reconcile_channel', 'live_rtmp', 'live_status_sync', 'telegram_stats']) {
      const res = stubRes();
